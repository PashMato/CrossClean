...
public class CameraOH extends GameObject implements GLSurfaceView.Renderer {
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        if (objectGroups != null) {
            for (List<GameObject> list : objectGroups) {
                if (list == null) {
                    continue;
                }
        ...
        // Make sure we don't get a massive dt
    ...
    protected void onDelete() {
        ...
        for (int i = 0; i < objectGroups.length; i++) {
            for (GameObject gameObject : objectGroups[i]) {
                gameObject.onDelete();
            }
            objectGroups[i] = null;
        }
    ...
    protected void handleObjectGroups() {
        ...
        for (List<GameObject> group : objectGroups) {
            ...
            length = g.model3D.Triangles.length / 3;

            /// ------> IMPORTANT DISCLAIMER <------
            ///  the next chuck of code isn't affected by "is active" because the first object
            ///  can be unactive but we still need to draw the rest of the array
            // Bind texture
            GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, g.textureId);
            ...
            for (int j = 0; j < group.size(); j++) {
                ...
                drawInObjectGroup(g.modelMatrix, g.rotationScaleMatrix, g.textureId, length);
            }
        }
    }
